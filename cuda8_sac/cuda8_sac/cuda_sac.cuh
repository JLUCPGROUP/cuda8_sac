...
float BuidBitModel32bit(HModel * hm) {
	...
	cudaMemcpy(d_bitDom, h_bitDom, H_VS_SIZE * sizeof(u32), cudaMemcpyHostToDevice);
	cudaMemcpy(d_MVarPre, h_MVarPre, H_VS_SIZE * sizeof(int), cudaMemcpyHostToDevice);
	cudaMemcpy(d_var_size, h_var_size, H_VS_SIZE * sizeof(u32), cudaMemcpyHostToDevice);
	cudaDeviceSynchronize();
	...
	for (int i = 0; i < H_CS_SIZE; ++i) {
		...
		for (int j = 0; j < c->size; ++j) {
			...
			h_bitSup[idx.y].y |= MASK1[t.x & U32_MOD_MASK];
		}
#ifdef SHOWDATA
		for (int j = 0; j < H_MDS; ++j) {
			const int idx = c->id * H_BITSUP_INTSIZE + j;
...
////	bool full() const { return top_ == max_size_; }
...
//
//
//class CacheStack :AssignedStack {
//public:
//	CacheStack() {};
//
//	virtual void initial(HModel* m, const int stack_size = MAXCACHESIZE) {
//		m_ = m;
//		vals_.resize(stack_size);
//		max_size_ = stack_size;
//	};
//
//	virtual void push(IntVal& v_a) {
//		vals_[top_] = v_a;
...
//			for (size_t j = 0; j < mds_; j++)
//				for (size_t k = 0; k < vs_size_; k++)
//					bsd_[i][j][k] = h_bitSubDom[GetBitSubDomIndexHost(i, j, k)];
//
//		//初始化中间变量
