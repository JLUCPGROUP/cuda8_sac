...
// 除法向上取整
__inline__ __device__ __host__ int GetTopNum(int x, int y) {
...
__global__ void showScope(int3* scope, int len) {
	...
	if (idx < len) {
		printf("c_id: %d, %d, %d\n", idx, scope[idx].x, scope[idx].y, scope[idx].z);
	}
}
...
__global__ void CsCheckMain(int* mConEvt, int* mVarPre, int3* scope, uint2* bitDom, uint4* bitSup) {
	//线程块id = 约束id，即每个线程块分得一个约束，每个约束跨两个线程束
	const int bid = blockIdx.x;
	//获得局部线程id
	const int tid = threadIdx.x;
	//线程束id
	...
	if (tid == 0) {
		...
		s_scp = scope[c_id];
		s_bitDom.x = bitDom[s_scp.x].x;
		s_bitDom.y = bitDom[s_scp.x].y;
		s_bitDom.z = bitDom[s_scp.y].x;
		s_bitDom.w = bitDom[s_scp.y].y;
	}
	__syncthreads();
...
float BuidBitModel64bit(HModel * hm) {
	...
	//////////////////////////////////////////////////////////////////////////
	//1.拷贝一些常量
	//H_DS_SIZE = hm->feature.ds_size;
	H_VS_SIZE = hm->property.vs_size;
	H_CS_SIZE = hm->property.ts_size;
	H_MDS = hm->property.max_dom_size;
	H_BD_INTSIZE = GetTopNum(H_MDS, U32_BIT);	// = 2
	//bitSup的最大长度 = 论域长度
	...
	cudaMemcpy(d_scope, h_scope, H_CS_SIZE * sizeof(int3), cudaMemcpyHostToDevice);

#ifdef SHOWDATA
	showScope << <H_MCCount, WORKSIZE >> > (d_scope, H_CS_SIZE);
#endif // SHOWDATA
	cudaDeviceSynchronize();

	//////////////////////////////////////////////////////////////////////////
	//3.变量论域[0, 64)
	h_bitDom = (uint2*)malloc(H_VS_SIZE * sizeof(uint2));
	h_MVarPre = (int*)malloc(H_VS_SIZE * sizeof(int));
	h_var_size = (int*)malloc(H_VS_SIZE * sizeof(int));
	cudaMalloc(&d_bitDom, H_VS_SIZE * sizeof(uint2));
	cudaMalloc(&d_MVarPre, H_VS_SIZE * sizeof(u32));
	cudaMalloc(&d_var_size, H_VS_SIZE * sizeof(u32));

	for (int i = 0; i < H_VS_SIZE; ++i) {
		HVar* v = hm->vars[i];
		const int dom_size = v->size;
		h_bitDom[i].x = UINT32_MAX;
		//最后一位偏移
		h_bitDom[i].y = UINT32_MAX << GetOffSet(dom_size % U32_BIT);
		h_MVarPre[i] = 1;
		h_var_size[i] = v->size;
	}

	cudaMemcpy(d_bitDom, h_bitDom, H_VS_SIZE * sizeof(uint2), cudaMemcpyHostToDevice);
	cudaMemcpy(d_MVarPre, h_MVarPre, H_VS_SIZE * sizeof(int), cudaMemcpyHostToDevice);
	cudaMemcpy(d_var_size, h_var_size, H_VS_SIZE * sizeof(int), cudaMemcpyHostToDevice);
	cudaDeviceSynchronize();
	...
	//////////////////////////////////////////////////////////////////////////
	//4.创建bitSubDom
	h_bitSubDom = (uint2*)malloc(H_VS_SIZE * H_MDS * H_VS_SIZE * sizeof(uint2));
	cudaMalloc(&d_bitSubDom, H_VS_SIZE * H_MDS * H_VS_SIZE * sizeof(uint2));
	...
	for (int i = 0; i < H_VS_SIZE; ++i) {
		for (int j = 0; j < H_MDS; ++j) {
			...
			for (int k = 0; k < H_VS_SIZE; ++k) {
				...
				if (j < h_var_size[i]) {
					//bitDom[k]有问题
					h_bitSubDom[idx].x = h_bitDom[k].x;
					h_bitSubDom[idx].y = h_bitDom[k].y;
				}
				else {
					h_bitSubDom[idx].x = 0;
					h_bitSubDom[idx].y = 0;
	...
	cudaMemcpy(d_bitSubDom, h_bitSubDom, H_VS_SIZE * H_MDS * H_VS_SIZE * sizeof(uint2), cudaMemcpyHostToDevice);
	...
	//////////////////////////////////////////////////////////////////////////
	//5.创建bitSup
	h_bitSup = (uint4*)malloc(H_CS_SIZE * H_MDS * sizeof(uint4));
	cudaMalloc(&d_bitSup, H_CS_SIZE * H_MDS * sizeof(uint4));

	for (int i = 0; i < H_CS_SIZE; ++i) {
		HTab* c = hm->tabs[i];
		HVar* v[2] = { hm->vars[c->scope[0]], hm->vars[c->scope[1]] };
		//初始化位矩阵
		for (int j = 0; j < H_MDS; ++j) {
	...
	cudaMemcpy(d_MConPre, h_MConPre, H_CS_SIZE * sizeof(int), cudaMemcpyHostToDevice);
	cudaMemcpy(d_MConEvt, h_MConEvt, H_CS_SIZE * sizeof(int), cudaMemcpyHostToDevice);
